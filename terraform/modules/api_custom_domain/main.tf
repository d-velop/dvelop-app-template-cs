...
resource "aws_route53_record" "stage" {
  ...
  alias {
    name                   = "${element(aws_api_gateway_domain_name.stage.*.regional_domain_name,count.index)}"
    zone_id                = "${element(aws_api_gateway_domain_name.stage.*.regional_zone_id,count.index)}"
