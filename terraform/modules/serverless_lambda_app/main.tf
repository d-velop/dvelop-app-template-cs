...
resource "aws_lambda_alias" "alias" {
  ...
  name             = "${replace(element(keys(var.stages), count.index),"/^[a-zA-Z0-9]*_/", "")}"
  description      = "lambda function version to use for ${replace(element(keys(var.stages), count.index),"/^[a-zA-Z0-9]*_/", "")}"
...
resource "aws_api_gateway_deployment" "deployment" {
  ...
  depends_on = [
    "aws_api_gateway_integration.proxyresource_method_integration",
  ]
